...
async function searchPeople(args) {
    ...
    if (args.person_titles)
        body.person_titles = args.person_titles;
    if (args.person_seniorities)
        body.person_seniorities = args.person_seniorities;
    if (args.person_locations)
        body.person_locations = args.person_locations;
    if (args.organization_locations)
        body.organization_locations = args.organization_locations;
    if (args.organization_num_employees_ranges)
        body.organization_num_employees_ranges = args.organization_num_employees_ranges;
    if (args.q_organization_domains_list)
        body.q_organization_domains_list = args.q_organization_domains_list;
    if (args.q_keywords)
        body.q_keywords = args.q_keywords;
    if (args.contact_email_status)
        body.contact_email_status = args.contact_email_status;
    if (args.currently_using_any_of_technology_uids)
        body.currently_using_any_of_technology_uids = args.currently_using_any_of_technology_uids;
    ...
    body.per_page = Math.min(args.per_page || 25, 100);
    const result = await apolloRequest("/mixed_people/api_search", "POST", body);
    // Check for warnings
...
async function searchCompanies(args) {
    const body = {};
    if (args.q_organization_name)
        body.q_organization_name = args.q_organization_name;
    if (args.organization_locations)
        body.organization_locations = args.organization_locations;
    if (args.organization_not_locations)
        body.organization_not_locations = args.organization_not_locations;
    if (args.organization_num_employees_ranges)
        body.organization_num_employees_ranges = args.organization_num_employees_ranges;
    if (args.currently_using_any_of_technology_uids)
        body.currently_using_any_of_technology_uids = args.currently_using_any_of_technology_uids;
    if (args.q_organization_keyword_tags)
        body.q_organization_keyword_tags = args.q_organization_keyword_tags;
    if (args.revenue_range_min || args.revenue_range_max) {
        body["revenue_range[min]"] = args.revenue_range_min;
        body["revenue_range[max]"] = args.revenue_range_max;
    }
...
async function enrichPerson(args) {
    const body = {};
    if (args.email)
    ...
    if (args.id)
        body.id = args.id;
    if (args.reveal_personal_emails)
        body.reveal_personal_emails = args.reveal_personal_emails;
    return apolloRequest("/people/match", "POST", body);
}
async function enrichCompany(args) {
    const domain = args.domain;
    return apolloRequest(`/organizations/enrich?domain=${encodeURIComponent(domain)}`, "GET");
}
async function enrichCompanyByName(args) {
    const companyName = args.company_name;
    ...
    const searchResult = await apolloRequest("/mixed_companies/search", "POST", searchBody);
...
async function bulkEnrichCompanies(args) {
    const companyNames = args.company_names;
    const locationHint = args.location_hint;
    const results = [];
...
// Main server
const server = new Server({ name: "apollo-mcp-server", version: "1.0.0" }, { capabilities: { tools: {} } });
// List tools handler
